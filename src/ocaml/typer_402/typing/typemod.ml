...
module StringSet =
  Set.Make(struct type t = string let compare (x:t) y = Pervasives.compare x y end)
...
and transl_signature env sg =
  ...
  let rec transl_sig env sg =
    ...
    match sg with
      [] -> [], [], env
    | item :: srem ->
      try transl_item env item srem
      with exn ->
        raise_error exn;
        transl_sig env srem

  and transl_item env item srem =
...
let check_nongen_scheme env sig_item =
  ...
  | Sig_module (_id, md, _) ->
      if not (closed_modtype md.md_type) then
